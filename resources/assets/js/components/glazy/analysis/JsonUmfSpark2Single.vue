    ...
    <tbody v-if="isLoaded">
        ...
        <tr class="oxide-colors">
            <td class="umf-spark-legend" v-if="showLegend">
                ...
                <br/>
                {{ Math.round(umf.R2OTotal * 100) / 100 }}:{{ Math.round(umf.ROTotal * 100) / 100 }}
            </td>
            <td class="umf-spark-row" v-bind:style="{ width: 5 + squareSize + 'px' }">
                <div v-if="umf.R2OTotal > 0"
                     v-bind:tooltip="'R2O:RO ' + Number(umf.R2OTotal).toFixed(2) + ':' + Number(umf.ROTotal).toFixed(2)"
                     class="umf-spark umf-spark-r2o color-R2O"
                     v-bind:style="{ height: squareSize + 'px', lineHeight: squareSize + 'px', width: (umf.R2OTotal * squareSize) + 'px' }">
                </div>
                <div v-if="umf.ROTotal > 0"
                     v-bind:tooltip="'R2O:RO ' + Number(umf.R2OTotal).toFixed(2) + ':' + Number(umf.ROTotal).toFixed(2)"
                     class="umf-spark umf-spark-ro color-RO"
                     v-bind:style="{ height: squareSize + 'px', lineHeight: squareSize + 'px', width: (umf.ROTotal * squareSize) + 'px' }">
                </div>
            </td>
            <td class="umf-spark-row"
                v-if="umf.B2O3 > 0">
                <div v-for="item in myB2O3"
                     v-bind:tooltip="'B2O3 ' + Number(umf.B2O3).toFixed(2)"
            ...
            <td class="umf-spark-legend" v-if="showLegend">
                ...
                {{ Number(umf.SiO2Al2O3Ratio).toFixed(2) }}
            </td>
            <td class="umf-spark-row">
                <div v-for="item in myAl2O3"
                     v-bind:tooltip="'Al2O3 ' + Number(umf.Al2O3).toFixed(2)"
            ...
            <td class="umf-spark-row">
                <div v-for="item in mySiO2"
                     v-bind:tooltip="'SiO2 ' + Number(umf.SiO2).toFixed(2)"
            ...
            <td class="umf-spark-row">
                <div v-for="item in otherOxides"
                     v-bind:tooltip="item.name + ' ' + Number(umf[item.name]).toFixed(2)"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      myB2O3: function () {
        var units = []
        for (var i = 0, len = Math.floor(this.umf.B2O3); i < len; i++) {
          units[i] = { width: this.squareSize + 'px' }
        }

        if (this.umf.B2O3 % 1 > 0) {
          units.push({ width: ((this.umf.B2O3 % 1) * this.squareSize).toFixed(1) + 'px' })
        }
      ...
      myAl2O3: function () {
        var units = []
        for (var i = 0, len = Math.floor(this.umf.Al2O3); i < len; i++) {
          units[i] = { width: this.squareSize + 'px' }
        }

        if (this.umf.Al2O3 % 1 > 0) {
          units.push({ width: ((this.umf.Al2O3 % 1) * this.squareSize).toFixed(1) + 'px' })
        }
      ...
      mySiO2: function () {
        var units = []
        for (var i = 0, len = Math.floor(this.umf.SiO2); i < len; i++) {
          units[i] = { width: this.squareSize + 'px' }
        }

        if (this.umf.SiO2 % 1 > 0) {
          units.push({ width: ((this.umf.SiO2 % 1) * this.squareSize).toFixed(1) + 'px' })
        }
      ...
      otherOxides: function () {
        ...
        for (var i = 0; i < Analysis.OTHER_OXIDES.length; i++) {
          var oxideValue = this.umf[Analysis.OTHER_OXIDES[i]]
          if (Number(oxideValue).toFixed(2) > 0) {
            while (oxideValue >= 1) {
              units.push({
